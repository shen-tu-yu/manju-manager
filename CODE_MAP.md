...
**「最近常用」规则（已定，改前先问）**：入库 **≥2 次**的位置置顶（标「用过 N 次」）；
...
> 自动退到 ③④ —— 功能不中断，只是列表短一些（本项目的 AI 沙箱就是这样，实机不受影响；
...
| 平台 | 来源 | 关键选择器 |
|---|---|---|
| 豆包 | DevTools 实测截图 | 输入框 `textarea.semi-input-textarea`；发送 `#flow-end-msg-send` / `[data-testid="chat_input_send_button"]` / `button[aria-label="发送"]`；停止 `[data-testid="chat_input_stop_button"]`（未实测，靠通用兜底） |
...
- **文件列表必须显示成树**：`buildSkillTree()` 按 `rel` 里的 `/` 在前端构造层级
  ...
  **目录可折叠（▾/▸）+ 按层缩进 + 只显示自己的文件名**。
  ⚠️ **不要退回平铺**（把 `子目录/文件.md` 一行一条列出来）—— 明确要求层级感，已被提过两次
...
- **指令由前端组装**（`buildAskText`）：走**预设库**，而且**分两类**（硬性要求）：
  ...
  **分镜预设**（"我改过的预设"）并清空老字段，**不丢已经改过的内容**；迁移幂等
...
- **分镜的口径是"大分镜"**（明确纠正过一次）：`{{seconds}}`（10s/15s）是**豆包这类视频模型
  一次能生成的时长选项**，不是小镜头长度。要求 AI：先按剧情排戏 → 按「总时长 ÷ 单次时长」
  酌情分成若干**大分镜**（60 秒 → 4 个 15 秒）→ **每个大分镜内部**再写多个小分镜/节拍。
  **脚本按 `{{split}}` 截出来的就是这几个大分镜**，每个正好对应一次豆包生成。
  ⚠️ **总时长不在工作台配** —— 剧情或 skill 里写了就按它，没写让 AI 自己判断（明确说过不需要预设）
  ⚠️ **给 AI 的指令必须消除歧义，光说"多个小分镜"不够**：真实翻车过一次 ——
  预设里写"内部要写清多个小分镜/动作节拍"，DeepSeek 理解成"一个长镜头内部的节拍"，
  每段都写成「单镜头 one-shot 无剪辑」。修法是**三件套**：
  ① 用对比句写死（"是「多镜头切换」，**不要**写成「单镜头 one-shot 无剪辑」"）
  ② 给出**结构样例**（`【小分镜】1. 0-5s｜景别+机位+运动：…`），照它写
  ③ 明确**冲突优先级**（"参考资料里的默认时长以这里的为准"、"与参考资料冲突时以本要求为准"）
  —— 因为 skill 文件里往往写着"默认单镜头 / 4-15 秒"这类会跟需求打架的规则
  ⚠️ **小分镜数量禁止写死**（被否掉过一次）：预设里一度写"必须由 3~5 个小分镜组成"，
  后来改成**内容驱动** ——"动作密集就多拆、一个连贯长动作
  也可以只用一两个镜头，不要为了凑数量硬拆"。**只约束"不能整段塞成一个镜头"，不约束拆几个。**
- 编排在 `onDeliverEvent` 里：`ask done` → 2.5 秒后入队 `read` → `read done` 带 `result` →
  **① 先 `setRawReply()`** 把**完整原文**贴进「📄 原文」窗口（**分割前先落地**）→
  **② 再 `splitStoryboard()` 切条** → `applyStoryboard()` **自动落条目**（默认替换，留底可撤销）；
  想看/想改再点工作台的「查看」→ `openStoryboardReview()`（勾选 + 可改 + 替换/追加）
- **📄 原文窗口**（`#rawWindow` / `.sbraw`，`openRawWindow()` / `splitFromRawWindow()`）——
  **"先把整个文本贴上去，再分割"**（硬性要求）。规则：
  · 分割**只读**窗口里的文本、不回写 → 原文不会被切分/落条目这些动作改掉
  · 新一轮生成的原文**不覆盖**旧的：旧的推进 `board.rawHistory`（`RAW_HISTORY_MAX` = 8 份），
    窗口左上角下拉可切回任意一份；原文与历史都持久化进 `promptBoard`，刷新/重开还在
  · 窗口里能直接改文本（改完点「✂ 分割成条目」就按改过的切），也支持**自己粘一段进来分割**（不生成也行）
  · **层级树** `describeRaw()`：识别出几个大分镜、每个里头几个小分镜（"层级要明确"）
  · ⚠️ **层级 1090 必须高于工作台 1080**：要求"层级要明确、不能被覆盖"（见 3.13）
  · 切不开时**不再弹旧的 `showRawReply` 模态**（已删）：提示落在窗口底部 `setRawFoot()`，
    原文就在眼前，改完再切 —— 同一个动作只留一个实现
...
### 3.13 浮层层级（`public/style.css` 里所有 `z-index` 的唯一依据）

界面全是浮层（工作台、灯箱、模态框、进度条、toast），**层级只能按这张表取，不许随手写数字**。
`style.css` 的 `#modalMask` 上面抄了一份同样的顺序，两处要一起改。

| 层 | z-index | 谁 |
|---|---|---|
| 下拉 | 800 | `#logPanel` 之类的下拉 |
| 拖拽上传遮罩 | 980 | `#dropMask` —— **故意低于工作台**：拖文件到工作台条目上时，面板要看得清 |
| 浮层面板 | 1080 | `.pboard`（提示词工作台） |
| **原文窗口** | **1090** | `.sbraw`（`#rawWindow` 📄 分镜原文）—— **必须高于工作台**：它是"先贴完整原文再分割"的落点，被工作台盖住就等于没有（要求"层级要明确、不能被覆盖"） |
| 入库卡片 | 1099 / 1100 | `.ig-mask` / `.ingest-card` |
| 右键菜单 | 1150 | `#ctxMenu` |
| 全屏看图 | 1190 | `#lightbox` |
| **模态框** | **1200** | `#modalMask` + `#modalBox` —— **必须盖住所有面板** |
| 上传进度 | 1250 | `.up-bar` |
| 提示 | 1300 | `#toasts` |
| 拖拽浮标 | 2000 | `.drag-ghost`（跟鼠标走，必须最上） |

⚠️ **踩过的坑**：`#modalMask` 原来是 **950**、工作台是 **1080** —— 工作台一开着，
弹窗就渲染在面板**背后**，用户点「⚙ 预设」只看到"什么都没弹出来"。
...
要求：**所有弹窗都照这个来** —— 所以位置尺寸**一律由这一套管，CSS 不许再写死**：
...
- **高度 `0` = 自适应**（模态框内容长短不一）；手动拉过高度就把 CSS 的 `max-height` 让开（`none`）
...
9. **AI 网页会把 Markdown 标记渲染掉，抓 `innerText` 拿不到 `###`** ——
   这是"取回几千字却没有 `###`、切不出分镜"的**真正原因**：
   指令里让 AI 用 `###` 分段，但页面把 `###` 渲染成了 `<h3>`，`innerText` 里只剩标题文字，
   **那三个 # 已经不存在了**。**一共栽过两次**，所以现在是四层防线：
   - **① 抓 Markdown 原文**：`readByCopy()` 点页面的「复制」按钮拿**原文**（最保真）。
   ...
   - **③ 内容兜底（第二次栽的修法）**：`splitStoryboard()` 认**两种**边界 ——
     ...
     光靠 ①② 不够：第二次真实失灵时 3 个分镜全被渲染成普通文本、`###` 一个不剩，
     `h1~h6` 也补不回来 —— **只认 `###` 就必然把整篇塞成一条**
   - **④ 不许静默**：`countBigShots()` 数出「文本里有 ≥2 个大分镜，却只切出 1 条」时
     **弹原文报错、不落条目**。第一次栽的教训就是"一声不响塞成一条大杂烩"，
     看到的只有"分割失灵"，看不到取回了什么
   —— 另外**预设侧也在配合**：要求 `###` **紧跟**标题文字不留空格
   （`###大分镜1｜…` 在 CommonMark 里不是标题，渲染成普通文本，标记就不会被吃掉；
   `### 大分镜1｜…` 带空格才会被当标题渲染掉）。
   **每次 read 都会把 `readProbe()` 随回执写进 debug.log**（复制按钮找到没 / 标题标签是什么 /
   DOM 文本含不含 `###` / 几条大分镜行）—— 下次再出问题，先看这几行，别猜
...
11. **"文本 N 秒没变"不能证明生成结束 —— 别用计时猜** ——
   **实测：只有思考链阶段界面在"动"，正式生成阶段界面不推。** 这句话同时打脸两头：
...
| **点了按钮"什么都没弹出来"** | **十有八九是 z-index 被浮层面板盖住了**，不是没弹 —— 先看 **3.13 浮层层级**（真实踩过：点「⚙ 预设」弹在工作台背后） |
| 提示/进度条看不见 | 同上：`#toasts` / `.up-bar` 必须高于 `.pboard` |
