...
- **分镜的口径是"大分镜"**（用户明确纠正过一次）：`{{seconds}}`（10s/15s）是**豆包这类视频模型
  ...
  **脚本按 `{{split}}` 截出来的就是这几个大分镜**，每个正好对应一次豆包生成。
  ...
  ⚠️ **小分镜数量禁止写死**（用户明确否掉过一次）：预设里一度写"必须由 3~5 个小分镜组成"，
  用户回"你也不能必须 3~5 给小分镜呀"。现在是**内容驱动** ——"动作密集就多拆、一个连贯长动作
...
- **📄 原文窗口**（`#rawWindow` / `.sbraw`，`openRawWindow()` / `splitFromRawWindow()`）——
  **"先把整个文本贴上去，再分割"**（用户要求）。规则：
  ...
  · ⚠️ **层级 1090 必须高于工作台 1080**：用户原话"层级要明确不能又被覆盖"（见 3.13）
...
| **原文窗口** | **1090** | `.sbraw`（`#rawWindow` 📄 分镜原文）—— **必须高于工作台**：它是"先贴完整原文再分割"的落点，被工作台盖住就等于没有（用户明确要求"层级要明确不能又被覆盖"） |
