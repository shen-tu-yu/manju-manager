...
| 1362 | POST | `/api/deliver/done` |
| 1387 | POST | `/api/deliver/queue` |
| 1425 | POST | `/api/deliver/send` |
| 1432 | GET | `/api/deliver/state` |
| 1442 | GET | `/api/skills/file` |
| 1460 | GET | `/api/fs/drives` |
| 1464 | GET | `/api/fs/dirs` |
| 1483 | GET | `/api/list` |
| 1515 | GET | `/api/tree` |
| 1537 | GET | `/api/file` |
| 1543 | GET | `/api/text` |
...
| 1567 | PUT | `/api/upload` |
| 1596 | POST | `/api/mkdir` |
| 1608 | POST | `/api/mkdir-template` |
| 1625 | POST | `/api/rename` |
| 1640 | POST | `/api/rename-batch` |
| 1673 | POST | `/api/move` · `/api/copy` |
| 1708 | POST | `/api/delete` |
| 1728 | GET | `/api/trash` |
| 1747 | POST | `/api/trash/restore` |
| 1773 | POST | `/api/trash/purge` |
| 1788 | GET | `/api/search` |
| 1831 | GET | `/api/vgroups` |
| 1836 | POST | `/api/vgroups` |
| 1855 | POST | `/api/vgroups/update` |
| 1867 | POST | `/api/vgroups/assign` |
| 1885 | POST | `/api/vgroups/delete` |
| 1899 | POST | `/api/vgroups/materialize` |
| 1956 | GET | `/api/duplicates` |
| 1975 | POST | `/api/clipboard` |
| 2020 | POST | `/api/reveal` |
| 2038 | GET | `/api/sysinfo` |
...
| 1061 | `queueDeliver()` |  |
| 2084 | `browserInfo()` | 给网页设置面板用：当前选的是谁 + 自动发现的本机浏览器列表 |
...
| 2785 | `buildAskText()` | 把预设指令渲染成真正要投出去的那段文字 |
| 2794 | `openAskTemplateEditor()` | 点开就能改的预设提示词 |
| 2822 | `splitStoryboard()` | 把 AI 的回复按 ### 切成一条条 |
| 2830 | `setGenState()` |  |
| 2837 | `syncGenButtons()` | 「查看」「撤销」只在有内容时出现 |
| 2847 | `applyStoryboard()` | 把切好的分镜落进条目（**自动**；留底供撤销） |
| 2860 | `undoStoryboard()` |  |
| 2870 | `generateStoryboard()` |  |
| 2892 | `trimBeforeFirstToken()` | 去掉第一个 ### 之前的杂质（思考过程有时和回答在同一段文本里） |
| 2899 | `showRawReply()` | 切不出分镜时，把取回的原文摊出来看 —— 比翻日志快，也不用猜 |
| 2915 | `openStoryboardReview()` | 切好的分镜先给用户过一遍：勾选 + 可改 + 选替换还是追加 |
| 2952 | `showModal()` |  |
| 2958 | `closeModal()` |  |
| 2965 | `openSettings()` |  |
| 3087 | `checkMjaInstalled()` | / |
| 3116 | `openDeliverPanel()` | / |
| 3217 | `openHelp()` |  |
| 3245 | `onSearchInput()` |  |
| 3257 | `doSearch()` |  |
| 3278 | `showCtxMenu()` |  |
| 3338 | `renderCtxMenu()` |  |
| 3357 | `showTreeCtxMenu()` | 左侧目录树的右键菜单（真实文件夹 / 虚拟分类 / 散-未归类 / 根目录） |
| 3390 | `renamePathByPath()` | 重命名任意文件夹（树里右键用） |
| 3406 | `deletePathByPath()` | 删除任意文件夹到回收站（树里右键用） |
| 3423 | `hideCtxMenu()` |  |
| 3425 | `revealInExplorer()` |  |
| 3432 | `copyToClipboard()` | 把选中的文件按 Windows 文件格式放进系统剪贴板，之后可在任意程序里 Ctrl+V |
| 3441 | `checkDuplicates()` |  |
| 3457 | `showDragGhost()` |  |
| 3466 | `moveDragGhost()` |  |
| 3472 | `hideDragGhost()` |  |
| 3476 | `setDropHints()` |  |
| 3482 | `clearDropTargets()` |  |
| 3490 | `isFileDrag()` | / |
| 3507 | `dragKind()` | / |
| 3518 | `bindEvents()` | / |
| 3529 | `bindToolbarEvents()` | 左树 ＋ / 视图切换 / 排序 / 筛选 / 缩放 / 滚动加载 |
| 3559 | `bindLogEvents()` | 操作日志面板：开关、清空、点外部关闭 |
| 3584 | `bindNavEvents()` | 导航按钮 / 侧栏按钮 / 搜索框 |
| 3606 | `bindContentEvents()` | 内容区：单击选中 / 双击打开 / 右键菜单 / 点空白关菜单 |
| 3657 | `bindOverlayEvents()` | 灯箱 / 模态遮罩 / 左侧分割条拖拽 |
| 3685 | `bindDragDropEvents()` | 拖拽：素材内部移动 + 外部文件拖入上传（从 bindEvents 拆出，纯搬迁） |
| 3828 | `bindKeyboardEvents()` | / |
| 3908 | `init()` |  |
...
| 2844 | `cloneItems` |  |
| 3067 | `DELIVER_TARGETS` |  |
| 3074 | `siteName` | 平台 id → 显示名（工作台/提示里用；注意它定义在工作台后面，但只在用户操作时调用，没问题） |
| 3077 | `DELIVER_KEY` | 记住上次选的投放平台（脚本一份通用，选哪个只是决定"打开"按钮开谁） |
| 3078 | `deliverSite` |  |
