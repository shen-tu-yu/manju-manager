...
const board = createBoard({ sendJSON, readJson: readJSONBody, DB });
...
const deliver = createDeliver({
  ...
  imagesMax: board.imagesMax,   // 每条提示词最多配几张图（由 lib/board.js 提供）
});
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (p === '/api/board' && req.method === 'GET') return board.get(req, res);

    if (p === '/api/board' && req.method === 'POST') return await board.save(req, res);
